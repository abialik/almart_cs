...
<div class="space-y-8 pb-10">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-gray-900 font-plus-jakarta">Edit Produk</h1>
            <p class="text-sm text-gray-500 mt-1">Ubah informasi stok atau detail produk.</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" required>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Harga (Rp) <span class="text-rose-500">*</span></label>
                            <input type="number" name="price" value="{{ old('price', $product->price) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" rows="5" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition">{{ old('description', $product->description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Ganti Gambar (Opsional)</label>
                        @if($product->image)
                            <div class="mb-2 w-20 h-20 rounded-xl overflow-hidden shadow-sm">
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer select-none">
                            <input type="checkbox" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="w-5 h-5 text-blue-600 rounded-lg border-gray-300 focus:ring-blue-500 transition">
